...
function simulateDayNotifications() {
  ...
  notifications.unshift(notifText); // most recent first
  renderNotifs();
  notifCount++;
...
function simulateJournaling() {
  ...
  logs.forEach((entry, i) => {
    setTimeout(() => {
      ...
      bubble.className = 'chat-bubble from-user fade-in bg-yellow-100';
      bubble.textContent = `[${entry.time}] ${entry.text}`;
...
window.onload = () => {
      ...
      datasets: [
        {
          ...
          data: [220, 300, 280, 310, 260, 330, 400],
          backgroundColor: '#34d399'
        },
        {
          label: 'Calorie Intake',
          data: [1800, 1900, 1700, 2000, 2100, 1950, 2200],
  ...
  if (sleepCanvas) {
    ...
    const moodMap = stressData.map(level => {
      if (level <= 3) return '😊';
      if (level <= 5) return '😐';
      return '😟';
    });
    ...
    if (moodTrend) {
      moodTrend.innerHTML = `<strong>Mood Trend:</strong> ${moodMap.join(' ')}`;
    }
    ...
    if (weeklySummary) {
      weeklySummary.innerHTML = `📊 <strong>Avg Sleep:</strong> ${avg(sleepData)} hrs &nbsp; | &nbsp; <strong>Avg Stress:</strong> ${avg(stressData)}/10`;
    }
...
//   if (e?.target?.id === 'chartModal' || e?.target?.tagName === 'BUTTON') {
//     document.getElementById('chartModal').classList.remove('active');
//   }
// }

// ✅ Place outside DOMContentLoaded or any function
function expandChart(id) {
